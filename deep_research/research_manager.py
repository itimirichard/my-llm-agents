...
class ResearchManager:
    ...
    async def run(self, query: str):
        ...
        with trace("Research Manager Trace", trace_id=trace_id):
            print(f"view trace: https://platform.openai.com/traces/{trace_id}")
            yield f"View trace: https://platform.openai.com/traces/{trace_id}"

            print("Planning research...")
            search_plan = await self.plan_searches(query)
            yield "Search plan generated, performing searches..."
            search_results = await self.perform_searches(search_plan)
            yield "Searches completed, writing report..."
            report_data = await self.write_report(query, search_results)
            yield "Report written, sending email..."
            await self.send_email(report_data)
